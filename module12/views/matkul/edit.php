<?php 
/** @var array $matkul */
$matkul = $matkul ?? ['kodeMK' => '', 'namaMK' => '', 'sks' => '']; 
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <title>SIA - Edit Mata Kuliah</title>
    ...
    <div class="w-full max-w-md bg-white rounded-[2rem] shadow-xl border border-slate-100 overflow-hidden relative">
        ...
        <div class="relative overflow-hidden bg-gradient-to-br from-violet-600 via-purple-600 to-indigo-800 p-8 text-white text-center">
            ...
            <div class="relative z-10">
                <h2 class="text-2xl font-extrabold tracking-tight">Ubah Mata Kuliah</h2>
                <p class="text-violet-100/80 text-xs mt-1">Perbarui kurikulum MK Kode #<?php echo htmlspecialchars($matkul['kodeMK']); ?></p>
            </div>
        </div>

        <form action="index.php?page=matkul&action=edit&id=<?php echo htmlspecialchars($matkul['kodeMK']); ?>" method="POST" class="p-8 space-y-5">
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">Nama Mata Kuliah</label>
                <input type="text" name="namaMK" required value="<?php echo htmlspecialchars($matkul['namaMK']); ?>"
            ...
            <div>
                ...
                <input type="number" name="sks" min="1" max="6" required value="<?php echo htmlspecialchars($matkul['sks']); ?>"
                       class="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm focus:outline-none focus:border-violet-600 focus:ring-2 focus:ring-violet-500/10 transition-all bg-slate-50/50">
            </div>
